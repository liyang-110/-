...
<jsp:include  page="../include/header1.jsp" flush="true"/>
   ...
   <div class="f_news_list">
   		...
   		<table class="table table-condensed">
        	...
        	<tbody>
       <%
    	PageResVO pageVO=(PageResVO)session.getAttribute("companysSearch");
	  	if(pageVO!=null){
	          	...
	          	if(list!=null&&list.size()>0){
	            	...
	            	for(Iterator iter=list.iterator();iter.hasNext();){
	            		...
	            		"<td><a href='"+request.getContextPath()+"/companyInfo.do?id="+temp.get(0)+"'>"+temp.get(1)+"</a></td></tr>");
	             	}
	             }else{
	    ...
	    %>
   		...
   		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	     	<tr>
					...
					<span> 
					    <a href="<%=request.getContextPath()%>/multiQueryAction.do?method=companysSearch&queryType=companysSearch&savesign=savesign&flush=flush&directflag=-2">首页</a>
						<a href="<%=request.getContextPath()%>/multiQueryAction.do?method=companysSearch&queryType=companysSearch&savesign=savesign&flush=flush&directflag=-1">上一页</a>
						<a href="<%=request.getContextPath()%>/multiQueryAction.do?method=companysSearch&queryType=companysSearch&savesign=savesign&flush=flush&directflag=1">下一页</a>
						<a href="<%=request.getContextPath()%>/multiQueryAction.do?method=companysSearch&queryType=companysSearch&savesign=savesign&flush=flush&directflag=2">尾页</a>
					</span>
				</td>
			</tr>
       </table>
