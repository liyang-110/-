...
(function(a){
	a.fn.jdSlide=function(k){
		...
		var n=p.pics.length;
		var o=function(){
			var q="<ul style='position:absolute;top:0;left:0;'><li><a href='../../assets/slide/"+p.pics[0].href+"' target='_blank'><img src='../../assets/slide/"+p.pics[0].src+"' width='"+p.width+"' height='"+p.height+"' /></a></li></ul>";
			i.css({
				position:"relative"
			}).html(q);
			i.find("ul").css({
		...
		var j=function(){
			...
			for(var t=0;t<n;t++){
				...
				switch(p.type){
					...
					break;
					case"image":q="<img src='../../assets/slide/"+p.pics[t].breviary+"' />";
					default:break
				}
				s.push("<span class='");
				s.push(r);
				s.push("'><a href='../../assets/slide/");
		...
		var c=function(){
			var q=[];
			for(var r=1;r<n;r++){
				q.push("<li><a href='../../assets/slide/");
				q.push(p.pics[r].href);
				q.push("' target='_blank'><img src='../../assets/slide/");
				q.push(p.pics[r].src);
				q.push("' width='");
				q.push(p.width);
				q.push("' height='");
