    ...
    <div class="container">
    	<table class="table table-bordered table-hover">
            ...
            <%
             ...
             %>
            	<tr>
                <td><%=row.get("job_type") %></td>
                <td><a href="company/info.do?id=<%=row.get("company_id") %>&jobs=1" target="_blank"><%=row.get("job_name") %></a></td>
                <td><%=row.get("professional") %></td>
                <td><%=row.get("job_person_num") %>人</td>
                <td><%=row.get("education") %></td>
                <td><%=row.get("areas") %></td>
                <td><%=row.get("money") %></td>
                <td>
                <a href="javascript:;" onclick="openOrderJob('<%=row.get("company_id") %>','<%=row.get("job_id") %>');" class="btn btn-warning btn-small ">
                投简历
                </a>
                </td>
                </tr>
