				...
				<div class="col-md-8 c_left">
					<div class="l_header">
						<ul>
							...
							<li class="active"><a href="/student/sMessage.do?type=resumeNo">不合格</a><span class="l_span">|</span></li>
							<!--<li><a href="/student/sMessage.do?type=resumeSys">系统消息</a></li>-->
						</ul>
					</div>
					<div class="l_bottom">
						<c:forEach var="row" items="${resumeNoList }">
							<div>
							<ul class="l_b_ul">
								<li>${row.job_name }<span class="l_b_salary">${row.money }</span>
									<span class="l_b_time">${row.add_time }</span>
								</li>
								<li>${row.area_name }</li>
								<li><a class="comnamea" href="/jobfair/company.do?id=${row.company_id }">${row.company_name }</a>
									<span class="l_b_status">HR认为你不太符合岗位要求</span>
								</li>
							</ul>
							</div>
						</c:forEach>
						<c:if test="${empty resumeNoList}">
