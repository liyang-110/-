...
String recruit = request.getParameter("recruit")!=null?new String(request.getParameter("recruit").getBytes("ISO-8859-1"),"UTF-8"):"";
String job_id = (String)request.getAttribute("jobid")!=null?(String)request.getAttribute("jobid"):"";

String sms_count = request.getAttribute("sms_count")!=null?String.valueOf(request.getAttribute("sms_count")):"0";
String company_tel =company!=null ?company.getTel():""; 
...
<!--简历未通过  -->
  	<div id="dlg_failure1" style="display:none;">
  		...
  		<input name="system" value="2" type="hidden"/>
  		</form>
  		</div>
  		</div>
  	</div>
<!--发送面试通知  -->
  	<div id="dlg_interview" style="display:none;">
  		...
  		<%} %>
  		<textarea id="content" name="content" cols="50" rows="5" style="width:440px;">    <%=student.get("name")%>您好，我司已收到您的简历并邀请您参加面试，联系方式：<%=company_tel %>
  		</textarea>
  		<br/>
...
<!--面试通过  -->
  	<div id="dlg_reinterview" style="display:none;">
  		...
  		<%} %>
  		<textarea  id="content" name="content" cols="50" rows="5" style="width:440px;">    <%=student.get("name")%>您好，恭喜您通过面试！请您尽快与我们联系确认相关信息，联系方式：<%=company_tel %>，面试时间：
  		</textarea>
   			<%if("0".equals(sms_count)){%>
  				<label>
  					<input type="checkbox" name="system" value="2" checked="checked"/>
...
<!--通知复试  -->
  	<div id="dlg_offer" style="display:none;">
  		...
  		<%} %>
  		<textarea id="content" name="content" cols="50" rows="5" style="width:440px;">    <%=student.get("name")%>您好，恭喜您已被我司录用！请您尽快与我们联系确认相关信息，联系方式：<%=company_tel %>
  		</textarea>
  			<%if("0".equals(sms_count)){%>
  				<label>
  					<input type="checkbox" name="system" value="2" checked="checked"/>
