...
<%
		...
		*/						
		...
		String[] title = { "序号","下载资质","单位名称","招聘职位","就业岗位", "联系人", "联系电话"};
			...
			if(list!=null && list.size()>0 )
			{
				for(int i=0; i<list.size(); i++ )
				{
					...
					if(row!=null)
					{
						...
						sheet.addCell(label);
						label = new Label(1,i+2,"'"+row.get("img") );
						sheet.addCell(label);
						label = new Label(2,i+2,"'"+row.get("company_name") );
						sheet.addCell(label);
						label = new Label(3,i+2,"'"+row.get("job_name") );
						sheet.addCell(label);						
						label = new Label(4,i+2,"'"+row.get("job_person_num") );
						sheet.addCell(label);
						label = new Label(5,i+2,"'"+row.get("contact_person") );
						sheet.addCell(label);	
						label = new Label(6,i+2,"'"+row.get("contact_tel") );
						sheet.addCell(label);		
					}
				}
			}
