...
<%
		...
		*/						
		...
		String[] title = { "序号","面试状态","日期","学校", "姓名", "性别", "专业","用人单位"};
		...
		try {
			...
			if(list!=null && list.size()>0 )
			{
				for(int i=0; i<list.size(); i++ )
				{
					...
					if(row!=null)
					{
						label = new Label(0,i+2,row.get("rn").toString() );
						sheet.addCell(label);
						label = new Label(1,i+2,(String)row.get("status") );
						sheet.addCell(label);
						label = new Label(2,i+2,(String)row.get("add_time") );
						sheet.addCell(label);
						label = new Label(3,i+2,(String)row.get("school_name") );
						sheet.addCell(label);
						label = new Label(4,i+2,(String)row.get("name") );
						sheet.addCell(label);
						
						label = new Label(5,i+2,(String)row.get("sex") );
						sheet.addCell(label);
						label = new Label(6,i+2,(String)row.get("profession") );
						sheet.addCell(label);
						label = new Label(7,i+2,(String)row.get("company_name") );
						sheet.addCell(label);
						label = new Label(8,i+2,(String)row.get("grad_year") );
						sheet.addCell(label);
					}
				}
			}
