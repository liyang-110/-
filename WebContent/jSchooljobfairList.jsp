...
if(request.getParameter("school_id")!=null){
	school_id=request.getParameter("school_id").toString();
}

cDataControlA cdata = new cDataControlA();
String area_id=""; 
String area_name=""; 
area_id=cdata.GetFirfield("select area_id from tbl_school where school_id='"+request.getParameter("school_id")+"'");
...
String school_name=cdata.GetFirfield("select school_name from tbl_school where school_id='"+school_id+"'");
 %>
...
<title><%=job_fair_type_name%> -- <%=school_name %> -- <%=area_name%>站 -- <%=com.zhaopin.Config.getSiteTitle() %></title>
...
<jsp:include  page="./school/inc/header.jsp" flush="true">
	<jsp:param name="school_id" value="<%=school_id %>" />
...
<div class="container"  style="height:30px;">
        <ul class="breadcrumb">
          ...
          <li>
           ...
           <li><a href="<%=request.getContextPath() %>/substation.do?method=schoolindex&school_id=<%=school_id %>">
           <%=school_name %></a><span class="divider">&gt;</span>
          </li>
    ...
    <div class="container f_content">
       ...
       <%
    	PageResVO pageVO=(PageResVO)session.getAttribute("schmoreJobfair");
	  	if(pageVO!=null){
         ...
         <div class="info_card"><!-- info_card 招聘会卡片模板，可循环套用 -->
            ...
            <div class="line">主<span class="divider"></span>办：
            	<a href="<%=request.getContextPath()%>/substation.do?method=schoolindex&school_id=<%=temp.get(6).toString()%>"  target="_blank"><%=temp.get(2)%></a>
            </div>
		...
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	         ...
	         <div class="pagination pagination-centered f">
					...
					<span> 
					    <a class="first" href="<%=request.getContextPath()%>/multiQueryAction.do?method=jobfairSearch&queryType=schmoreJobfair&savesign=savesign&flush=flush&directflag=-2">首页</a>
						<a class="pre" href="<%=request.getContextPath()%>/multiQueryAction.do?method=jobfairSearch&queryType=schmoreJobfair&savesign=savesign&flush=flush&directflag=-1">上一页</a>
						<a class="next" href="<%=request.getContextPath()%>/multiQueryAction.do?method=jobfairSearch&queryType=schmoreJobfair&savesign=savesign&flush=flush&directflag=1">下一页</a>
						<a class="last" href="<%=request.getContextPath()%>/multiQueryAction.do?method=jobfairSearch&queryType=schmoreJobfair&savesign=savesign&flush=flush&directflag=2">尾页</a>
					</span>
					 </div> </div>
				</td>
			</tr>
