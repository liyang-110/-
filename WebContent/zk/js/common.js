...
// 返回参数及数据类型：无
/* ===================================================== */
function ajaxData(ins_id, inf_url, inf_deal, ini_page, ini_pagesize,
		ini_currentrow, tip) {
	var out_return;
	var ls_url = inf_url();
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (out_return.length > 0 && out_return[0].returncode == 'ok') {
				...
				out_return = "{\"total\":" + rowcount + ",\"rows\":"
						+ JSON.stringify(out_return[0].queryresult) + "}";
			...
			} else if (out_return.length > 0
					&& out_return[0].returncode == 'fail') {
		...
		$(pg).pagination(
				{
					...
					onRefresh : function(pageNumber, pageSize) {
						ajaxData(ins_id, inf_url, inf_deal, pageNumber,
								pageSize, undefined);
					},
...
// 返回参数及数据类型：
/* ===================================================== */
function linkSelectCode(ins_sqlid, ins_id, inb_checkone, ins_ld,
		ins_checkvalue, ins_isbuxian, height) {
	try {
		...
		if (js_array != undefined) {
			...
			} else if (ins_id.indexOf("area_id") > 0
					|| ins_id.indexOf("ad_type") > 0) {
				$("#" + ins_id).combobox({
					valueField : 'id',
					textField : 'text',
					panelHeight : '200'
	...
	var ls_url = cur_path
			+ "/esbServer.do?method=execClass&methodId=base_select_001&sqlId="
			+ ins_sqlid + "&arguments=" + ins_ld;
	var ls_return;
	var lo_data;
	$.ajax({
		url : ls_url,
		...
		success : function(data) {
			...
			if (ls_return.length > 0 && ls_return[0].returncode == 'ok') {
				...
				//alert(ins_checkvalue);
				if (typeof (ins_checkvalue) != "undefined"
						&& ins_checkvalue != "") {
					$("#" + ins_id).combobox('select', ins_checkvalue);
				}
...
function linkSelectCodeS(ins_sqlid, ins_id, inb_checkone, ins_ld,
		ins_checkvalue, ins_chagfun) {
	var ls_url = cur_path
			+ "/esbServer.do?method=execClass&methodId=base_select_001&sqlId="
			+ ins_sqlid + "&arguments=" + ins_ld;
	var ls_return;
...
function searchForm(ins_id, inf_url, inf_deal, ini_page, ini_pagesize,
		ini_currentrow, tip) {
	var ls_url = inf_url();
	...
	$('#fm').form(
			'submit',
			{
				...
				success : function(data) {
					...
					data = decodeURIComponent(data);
					out_return = eval("[" + data + "]");
					if (out_return.length > 0
							&& out_return[0].returncode == 'ok') {
						rowcount = out_return[0].querycount;
						if (rowcount < 1 && !tip) {
							//$.messager.alert('提示信息','无查询结果');   
						}
						...
						$(ins_id).datagrid('loadData', out_return[0]);
						initPaperareaForm(ins_id, inf_url, inf_deal,
								ini_pagesize);
						//选中当前行
						if (ini_currentrow >= 0) {
							$(ins_id).datagrid('selectRow', ini_currentrow);
		...
		$(pg).pagination(
				{
					pageSize : pageSize,
					onRefresh : function(pageNumber, pageSize) {
						searchForm(ins_id, inf_url, inf_deal, pageNumber,
								pageSize, undefined);
					},
					onChangePageSize : function(pageSize) {
						searchForm(ins_id, inf_url, inf_deal, 1, pageSize,
								undefined);
					},
					onSelectPage : function(pageNumber, pageSize) {
						searchForm(ins_id, inf_url, inf_deal, pageNumber,
