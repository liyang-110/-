...
$(function() {
		...
		$("#paymentStat").bind("click",function(e){
			e.preventDefault();
			paymentStat(); 
		});
	});
...
function getUrl() {
	var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=zk_pay_query&in_zk_id="+user_code;
	return ls_url;	
	}
/* =======================================================*/
// 函数作用：加载或刷新数据
...
function applyCash() {
    $('#if_deal')[0].src='jApplyCash.jsp';
    $('#dlg_deal').dialog({width:320,height:260,fit:false});
    $('#dlg_deal').dialog('open').dialog('setTitle','提现申请'); 
}
function paymentStat(){
	var in_add_time_1 = $("#in_add_time_1").datebox('getValue');
	var in_add_time_2 = $("#in_add_time_2").datebox('getValue');
	var queryStr="&method=execProc&methodId=zk_pay_query&in_add_time_1="+in_add_time_1+"&in_add_time_2="+in_add_time_2;
    $('#if_deal')[0].src='jStatWindow.jsp?in_r=zk&in_action=paystat'+queryStr;
    $('#dlg_deal').dialog({width:320,height:260,fit:false});
    $('#dlg_deal').dialog('open').dialog('setTitle','提现统计'); 
}