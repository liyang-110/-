...
<div class="area_select_link">
	<a <%=area_id==null?"class=\"active\"":"" %> href="?t=multiple">全国站</a>
					<%
                    ...
                    for( Iterator it=areaList.iterator();it.hasNext();)
                    {
                     ...
                     %>
                     <a class="active" href="?t=multiple&a=<%=row.get(0) %>"><%=row.get(1) %></a>
                     <% }else{ %>
                     <a href="?t=multiple&a=<%=row.get(0) %>"><%=row.get(1) %></a>
