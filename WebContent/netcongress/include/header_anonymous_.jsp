...
<div class="head">
    <div class="head_top">
    	<div class="head_left">
    		<p>
    		<a href="javascript:;" onclick="open_login_layer();">[&nbsp;<span>登录</span>&nbsp;</a>
        ...
        <div class="head_search" style="float:left;width:250px;">
        	<form name="head_search" onsubmit="return findInPage(this.string.value);">
            	<input name="string" type="text" size=15 onchange="n = 0;" placeholder="输入企业或职位名称" class="zp" />
...
<div class="container">
	<div class="top_a">
        ...
        <div class="top_text" >
            ...
            <p style="color:#ffe496; font-size:20px; line-height:23px;position:absolute; top:468px; left:512px;  width: 198px; text-align:center;">
              结束倒计时<br />
...
<div class="cont">
    <div class="left_suspend"> 
		...
		<% HashMap jobfairTree= (HashMap)request.getAttribute("jobfairTree"); 
			  ...
			  for(int i=0;i< (rowcount/8)+1 ; i++ ){
		 %>
		<dl  page="<%=i %>"  class="list  <%=(i==0)?"current":"" %>"  style="min-height:239px;<%=(i==0)?"":"display:none;" %>">
        <% 
				 for(;it.hasNext(); ){ 
		    ...
		    <dd num="<%=rownum%>">
				...
				<div class="nav_right">
					...
					%>
						<li style="display:block;" online="<%=jf.get("isonline")%>">
								<a href="/school/info.do?id=<%=jf.get("school_id")%>" target="_blank">
									<%=jf.get("school_name") %></a>
								<a href="/jobfairInfo.do?id=<%=jf.get("job_fair_id")%>" target="_blank">
									<span class="school_a">
										<%=jf.get("job_fair_name") %>
									</span>
								</a>
    ...
    <div class="con">
       ...
       <div class="ren" style="height:60px;">
            <div class="clear"></div>
       </div>      
    </div>
    <!--right_suspend
    <div class="right_suspend"> 
    	<p><a href="javascript:;" onclick="reload();" style="color:#FFF; font-weight:bold;">刷新</a></p>
        <dl class="list">
...
$(function(){
	var job_fair_id="<%=jobfair.get("job_fair_id")%>";
	if( !$.cookie("into+"+job_fair_id ) ){
		$.cookie("into+"+job_fair_id,true);
		open_agree_layer("wellcome");
	}
	$("a.notLogin").bind("click",function(e){
			e.preventDefault();
			open_login_layer();
	});
});
