...
$(function () {
	...
	$("#btn_return").click(function(){
			$.messager.confirm("提示信息","您确定要放弃保存编辑的海报？",function(r){
				if (r){
					window.close();
				}
			});
...
/* ===================================================== */
function save() {
	var ls_url = cur_path + "/esbServer.do?&method=execProc&methodId=poster_orders&in_action=add"+param+"&date=" + new Date();
   //处理数据 调用公用js保存数据函数 code:数据添加成功
	saveData(ls_url, closePage, "保存成功，页面将关闭！");
}
