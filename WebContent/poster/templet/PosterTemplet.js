	...
	$('#resultId').datagrid( {
		...
		columns : [ [ {
			...
			formatter : function(value,row,index){
				return '<a href="'+cur_path+'/poster/index.jsp?id='+row.tpl_id+'" target="_blank">'+value+'</a>';
			}
		},{
			field : 'tpl_pic',
			title : '预览',
			formatter : function(value,row,index){
				var cc="没有缩略图";
				if( value!=null && value!=''){
					cc= '<a href="'+cur_path+'/poster/index.jsp?id='+row.tpl_id+'" target="_blank">'
							+'<img class="tpl_preview" src="'+cur_path+'/'+value+'" />'
							+'</a>';
				}
				return cc;
			}
			...
			formatter : function(value,row,index){
				var cc="";
				if( value!=null && value!=''){
					cc= '<a href="'+cur_path+'/poster/index.jsp?id='+row.tpl_id+'" target="_blank">预览模版</a>';
					cc+='<br/>';
					//cc+= '<a href="'+cur_path+'/poster/orders/jPosterOrdersAdd.jsp?id='+row.tpl_id+'" target="_blank">创建海报</a>';
					cc+='<a href="javascript:;" onclick="openWindow(\''+cur_path+'/poster/orders/jPosterOrdersAdd.jsp?id='+row.tpl_id+'\')">创建海报</a> ';
				}
				return cc;
			}
		}
...
function getUrl() {
	var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=poster_templet&in_URLEncoding=UTF-8"+param;
	return ls_url;	
}
...
function view() {
    ...
    if(lo_selected.length!=1){
        ...
        $('#if_deal')[0].src=cur_path+'/company/jJobFairView.jsp?id='+ls_id+'&in_pagesize=' + li_pagesize+'&in_pagenum=' + li_pagenum+'&in_currentnum=' +li_currentnum +param+"&date="+new Date();
...
function add(){
	...
	$("#if_deal")[0].src = "./jPosterTempletAdd.jsp?pagesize=" + li_pagesize + "&pagenum=" + li_pagenum + "&currentnum=" + li_currentnum;
...
function edit() {
    ...
    if(lo_selected.length!=1){
        ...
        $('#if_deal')[0].src='./jPosterTempletEdit.jsp?id='+ls_id+'&in_pagesize=' + li_pagesize+'&in_pagenum=' + li_pagenum+'&in_currentnum=' +li_currentnum +param+"&date="+new Date();
		$('#dlg_deal').dialog('open').dialog('setTitle','编辑'); 
    }
}
function openWindow( url ){
	window.open(url);
	$.messager.confirm("提示信息","请在新窗口中编辑并保存海报！保存完成请点击确定！",function(r){
		    if(r){
