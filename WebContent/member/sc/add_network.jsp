...
$(function(){
	...
	$("#in_job_fair_time").val( initdatetime() );
});
function saveForm( form_id )
{
	var ls_url = "/esbServer.do?method=execProc&methodId=job_fair_03&in_r=sc&&in_school_id="+school_id+"&in_add_person="+user_code+"&date=" + new Date();
	$("#"+form_id ).form("submit",{
		url:ls_url,
		onSubmit:function(){
		...
		success:function( data ){
            ...
            if (re.length > 0 && re[0].returncode == 'ok') {
                 ...
                 msg_alert('提示信息',l_message,undefined,function(){//提示信息
                 	window.location.href="issue_network.jsp";
            	});
            } else {
...
<div class="header_main">
            ...
            <div class="pull_right">
                <div class="biankuang">
                	<div class="list_alter fanhui">
						<div class="pull-left">
                         	<span style="color:#FFF; font-weight:bold; font-size:16px;">发布校园招聘会</span>
                        </div>
                        <div class="pull-right">
                         	<a href="issue_network.jsp"><img src="img/fanhui.png"></a>
                        </div>
                    </div>
                    <!----------------------------返回------------------------------------->
                    <div class="form" style="width:780px;margin:20px 0; padding-left:50px;">
                    	<form id="fm" name="fm" method="post">
						<input value="netcongress" type="hidden" id="in_job_fair_type" name="in_job_fair_type" />
                    	...
                    	<table class="table">
                           <tbody>
                              ...
                              <!-- 
                               <tr>
                                 <td class="text_right">主办单位：</td>
                                 <td class="text_left">
                                   <input type="text" id="in_job_fair_sponsor" name="in_job_fair_sponsor" class="form-control easyui-validatebox" data-options="required:true,missingMessage:'例如：西京学院就业中心 西安理工大学经管系' " placeholder="">
                                 </td>
                              </tr>
                               -->
    <tr>
	...
	<tr>
		...
		<td colspan="2">
			<input type="hidden" id="in_area_id" value="<%=area_id %>," name="in_area_id" />
			<style>
				ul.area_id_list li {float:left; margin:0 15px;}
				ul.area_id_list .checkbox {margin:0;}
			</style>
			<script>
		  	 ...
		  	 <ul class="area_id_list" style="display:block;width:520px;">
		    <% 
		    ...
		    if (area_list!=null && area_list.size()>0){
		    	for(Iterator iter=area_list.iterator();iter.hasNext();){
			    	...
			    	if( templist.get(0).toString().equals( user.getAreaId() ) ){
			    	%>
			    	<li>
			    	<label class="checkbox"><input checked="checked" name="area_id" type="checkbox" value="<%=templist.get(0)%>"/><%=templist.get(1) %></label>
			    	</li>
			    	<%
			    	}else{
			    	%>
...
<%
 ...
 %>
<input id="in_job_fair_level" name="in_job_fair_level" class="easyui-validatebox" data-options="required:true,missingMessage:'请选择招聘会学历层次' " style="border:none;width:0px;" />           
                                    </div>
                                 </td>
                              </tr>
                              <tr>
                              ...
                              <tr><td colspan="3">
                              	  		<script id="job_fair_invitation"  name="job_fair_invitation"  type="text/plain" style="width:100%;height:280px;"></script>
 <script type="text/javascript">
    //实例化编辑器
    ...
    var ue = UE.getEditor('job_fair_invitation', {
    	...
    	,toolbars: [[
            ...
            'inserttable', 'deletetable', 'insertparagraphbeforetable', 'insertrow', 'deleterow', 'insertcol', 'deletecol', 'mergecells', 'mergeright', 'mergedown', 'splittocells', 'splittorows', 'splittocols'
                             ...
                             <tr>
								<td class="text_right">招聘会海报：</td>
								<td class="text_left">	
								<table><tr><td>
									<input class="form-control" name="in_poster_pic_name" type="text" readonly id="in_poster_pic_name" />
									<input type="hidden" name="in_poster_pic" id="in_poster_pic"/>
									<input type="hidden" name="in_poster_path" id="in_poster_path" />
									</td><td>
									<div id="poster_pic_upload_btn" style="line-height:1em;">
									</div>
									</td>
									<td>
									<span class="red">格式：305px*195px ，大小:<500KB。</span>
									</td>
									</tr>
								</table>
								</td>
                              ...
                              <tr>
                                 ...
                                 <td class="text_left">
                                    ...
                                    <a href="issue_network.jsp" class="btn btn-default anniu_a">取消</a>
                                 </td>
                              </tr>
                           </tbody>
                        </table>
