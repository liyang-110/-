...
/* ===================================================== */
function setData() {
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=card_query&in_action=view&in_r=ad&in_id=" + id + "&date=" + new Date();
	$.ajax({
		url:ls_url, 
		type:"post", 
		dataType:"text", 
		...
		if (re.length > 0 && re[0].returncode == "ok") {
			...
			$("#card_pic").attr("src","/"+initdata[0].in_card_img_path);
		}
	}});
...
function edit() {
	...
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=card_query&in_action=edit&in_r=ad&date=" + new Date();
	   //处理数据 code:数据修改成功
