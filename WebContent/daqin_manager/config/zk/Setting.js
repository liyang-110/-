...
/* ===================================================== */
function setData() {	
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=zk_config_update&in_update=query&date=" + new Date();
	$.ajax({
		url:ls_url, 
		type:"post", 
		dataType:"text", 
...
/* ===================================================== */
function save() {
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=zk_config_update&in_update=update&date=" + new Date();
	   //处理数据 code:数据修改成功
   var ins_msg="\u4FDD\u5B58\u6210\u529F\uFF01";
   $('#fm').form('submit', {
        url: ls_url,
