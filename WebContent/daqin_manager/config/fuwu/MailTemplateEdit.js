...
/* ===================================================== */
function setData() {	
	var ls_url = cur_path + "/esbServer.do?in_r=ad&method=execProc&methodId=zp_fuwu&in_action=view&in_fuwu_id=" + id + "&date=" + new Date();
	$.ajax({
		url:ls_url, 
		type:"post", 
		dataType:"text", 
...
function edit() {
	...
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=zp_fuwu&in_r=ad&in_action=edit&in_tpl_id=" + id+"&date=" + new Date();
	   //处理数据 code:数据修改成功
