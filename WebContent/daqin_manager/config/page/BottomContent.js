	...
	$('#resultId').datagrid( {
		...
		columns : [ [ {
			...
			formatter: function(value,row,index){
				return  '<a target="_blank" href="'+cur_path+'/page.do?id='+row.id+'">'+value+ '</a>';
			}
		},{
			field : 'add_time',
			title : '修改时间'
...
function getUrl() {
	var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=bottom_content_query&in_URLEncoding=UTF-8"+querystr;
	return ls_url;	
}
...
function add(){
	...
	$("#if_deal")[0].src = "./jBottomContentAdd.jsp?pagesize=" + li_pagesize + "&pagenum=" + li_pagenum + "&currentnum=" + li_currentnum;
...
function edit() {
    ...
    if(lo_selected.length!=1){
	    ...
	    li_currentnum = $("#resultId").datagrid("getRowIndex",lo_selected[0]);
	    ls_id     = lo_selected[0].id;
        lo_pg         = $("#resultId").datagrid("getPager");
        li_pagenum    = lo_pg.pagination('options').pageNumber;

        $('#if_deal')[0].src='./jBottomContentEdit.jsp?id='+ls_id+'&in_pagesize=' + li_pagesize+'&in_pagenum=' + li_pagenum+'&in_currentnum=' +li_currentnum +"&date="+new Date();
