...
/* ===================================================== */
function getUrl() {
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=news_cj_statistical_school&in_action=query&in_r=ad&date=" + new Date();
	return ls_url;	
	}
/* =======================================================*/
// 函数作用：删除（批量删除）
	    	 ...
	    	 $.messager.confirm('提示信息', '您确定要进行删除操作吗?', function(r){
	    	 	if (r){
	    	 		for(var i=0;i< lo_selected.length;i++){
   					...
   					ls_url = cur_path + "/esbServer.do?method=execProc&methodId=news_cj_statistical_school&in_r=ad&in_action=del" + ls_id+"&date="+new Date();
...
function assignAdviser()
{
    ...
    var ls_url;           //url
    var ls_sign="";
    lo_selected   = getCheckObj('resultId','请选择要分配的资源！'); 

    if(!lo_selected){
    	$.messager.alert('提示信息','请选择要分配的资源！','info');
    	return false;
    }else{
	    li_currentnum = $("#resultId").datagrid("getRowIndex",lo_selected[0]);
        lo_pg         = $("#resultId").datagrid("getPager");
        li_pagenum    = lo_pg.pagination('options').pageNumber;
	 		...
	 		$("#company_list2").html("");
	 		for(var i=0;i<lo_selected.length;i++){
	 			if(lo_selected[i].noassigned==0){
	 	    		$.messager.alert('提示消息','请选择未分配的资源','info');
	 	    		$("#resultId").datagrid("clearSelections");//清除选中行
	 	    		return false;
	 	    	}else{
	 	    		input='<li><input type="hidden" name="in_news_source_'+i+'" value="'+lo_selected[i].news_source+'"/>'+lo_selected[i].news_source+'</li>';
...
{
   var 	ls_url = cur_path + "/esbServer.do?method=execProc&methodId=news_cj_statistical_school&r=ad&in_action=chown&in_URLEncoding=UTF-8&date="+new Date();
