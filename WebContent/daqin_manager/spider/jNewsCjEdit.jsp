...
<%@page import="com.zhaopin.dao.factory.NewsCjModel"%>
<%@page import="org.apache.commons.beanutils.DynaBean" %>
<%@ taglib uri="TreeTag" prefix="t" %>
<%@ include file="../include/checkLogin.jsp" %>
<%
NewsCjModel model=new NewsCjModel();
String new_id= request.getParameter("id") ;
DynaBean bean= model.find( new_id   );
String content=""; 
String area_id="28";
if( bean!=null ){
	area_id = (String)bean.get("area_id");
	content=model.getNewsContent( new_id );
}else{
...
<script type="text/javascript" src="NewsCjEdit.js"  charset="utf-8"></script>
<script type="text/javascript">
...
<form id="fm" method="post"  name="fm">
	<input type="hidden" name="in_spider_id" id="in_spider_id" value="<%=request.getParameter("id") %>"/>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		...
		<td width="90%">
		<input value="<%=bean.get("news_title") %>" style="width:350px;" id="in_news_title" name="in_news_title" class="easyui-validatebox inputbox" required="true"/></td>
	</tr>
	<tr>
	...
	<tr>
		...
		<td><input value="<%=bean.get("news_author") %>" id="in_news_author" name="in_news_author" class="easyui-validatebox inputbox"/></td>
	</tr>
	<tr>
		<td class="tabright">来源：</td>
		<td><input value="<%=bean.get("news_source") %>" id="in_news_source" name="in_news_source" class="easyui-validatebox inputbox"/></td>
	</tr>
	<tr>
	...
	<input type="hidden" id="in_area_id" name="in_area_id" value="<%=area_id%>"/>
	<%
	        // [ id, name, pid checked ]
			...
			" left join "+
			" ( select "+area_id+" area_id from dual  ) b on a.id=b.area_id order by id asc";
	 %>
