...
$(function() {    
    ...
    $("#in_news_type").combobox({
 	...
 	setTimeout(function(){
	//	linkSelectCode('news_type03','in_sub_type',true,'','mingqizp');
	},500);
 	*/
	getData();
	ajaxdata1();
	...
	$('#resultId').datagrid( {
		...
		columns : [ [ {
			...
			formatter: function(value,row,index){
				return value+'<a href="'+row.url+'" target="_blank">来源</a>';
			},
			width:380 
		},{
			field : 'status',
...
/* ===================================================== */
function getUrl() {
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=spider_task&in_action=query&in_r=ad&date=" + new Date();
	return ls_url;	
	}
/* =======================================================*/
// 函数作用：删除（批量删除）
	    	 ...
	    	 $.messager.confirm('提示信息', '您确定要进行删除操作吗?', function(r){
	    	 	if (r){
	    	 		for(var i=0;i< lo_selected.length;i++){
    					ls_id += "&in_place_mgr"+i+"="+ lo_selected[i].place_id;
   					}
   					ls_url = cur_path + "/esbServer.do?method=execProc&methodId=place&in_r=sc&in_action=del" + ls_id+"&date="+new Date();
...
function edit() {
    ...
    if(lo_selected.length!=1){
	    ...
	    ls_id     = lo_selected[0].spider_url;
        lo_pg         = $("#resultId").datagrid("getPager");
        li_pagenum    = lo_pg.pagination('options').pageNumber;

        $('#if_deal')[0].src='jNewsEdit.jsp?id='+ls_id+'&in_pagesize=' + li_pagesize+'&in_pagenum=' + li_pagenum+'&in_currentnum=' +li_currentnum +"&date="+new Date();
	        ...
	        li_pagenum    = lo_pg.pagination('options').pageNumber;
   	 		...
   	 		for(var i=0;i< lo_selected.length;i++){
  				if( lo_selected[i].status==1){	
 						input='<li><input type="hidden" name="in_spider_'+i+'" value="'+lo_selected[i].spider_url+'"/>'+lo_selected[i].title+'</li>';
...
{
   var 	ls_url = cur_path + "/esbServer.do?method=execProc&methodId=spider_task&r=ad&in_action=publish&in_URLEncoding=UTF-8&date="+new Date();
