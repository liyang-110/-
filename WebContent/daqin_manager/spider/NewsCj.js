...
/* ===================================================== */
function getUrl() {
	var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=news_cj&in_action=query&in_r=ad&date=" + new Date();
	return ls_url;	
	}
/* =======================================================*/
// 函数作用：删除（批量删除）
    ...
    if(!lo_selected){
        ...
        li_pagenum    = lo_pg.pagination('options').pageNumber;
	 	li_pagesize = lo_pg.pagination("options").pageSize;
	 		...
	 		$("#company_list2").html("");
	 		for(var i=0;i<lo_selected.length;i++){
	 			if(lo_selected[i].admin_name!=null){
	 				$.messager.alert('提示信息','请选择未分配的资源！','info');
	 				$("#resultId").datagrid("clearSelections");
	 				return false;
	 		}else{ 
	 				input='<li><input type="hidden" name="in_spider_'+i+'" value="'+lo_selected[i].news_id+'"/>'+lo_selected[i].news_title+'</li>';
