...
function editSchoolName( school_name )
{
	var school_name_1=school_name;
	$("#in_school_name").val( school_name );
	$("#dlg_school_name").show().dialog({title:'修改学校名称',
												width: 350,
												height: 120,
												fit:false,
												resizable:true,
												buttons: [{
											                    ...
											                    handler:function(){
											                    	...
											                    	editSchoolNameHandle( school_name,school_name_1 );
											                    }
											                },{
											                    text:'取消',
											                    handler:function(){
											                        $("#dlg_school_name").dialog('close') ;
											                    }
											                }]
	});
	$("#in_school_name").dialog("open");
	//alert( school_name );
}
function editSchoolNameHandle( school_name,school_name_1 )
{
	if(!school_name ){
		$.messager.alert("提示信息","学校名称不能为空！","info");
		return;
	}
    var ls_url = cur_path + "/esbServer.do?in_URLEncoding=UTF-8&method=execProc&methodId=student_query&in_action=update_school&in_r=ad&in_school_name="+school_name+"&in_school_name_1="+school_name_1+"&date="+new Date();
...
function getData() {
	$('#resultId').datagrid( {
		...
		columns : [ [{
			...
			formatter :function(value,row,index){
				if( row.school_name ){
					var btn = '<button onclick="editSchoolName(\''+row.school_name+'\');">修改</button>';
					return btn;
				}else{
					return '';
				}
