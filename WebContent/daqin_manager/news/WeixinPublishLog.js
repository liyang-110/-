...
function getData() {
	$('#resultId').datagrid( {
		...
		columns : [ [
		{
			...
			width : 260,
			formatter : function(value,row,index){
				return '<a href="'+row.article_url+'" target="_blank">'+value+'</a>';
			}
		},{
			field : 'company_name',
			title : '公司名称',
...
function getUrl() {
	var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=ad_trade_weixin_log&in_r=ad&in_URLEncoding=UTF-8"+querystr;
	return ls_url;	
}
...
function inputPublish(){
    ...
    if(lo_selected.length!=1){
        ...
        $("#inputPublishForm").form("load",{
        	company_name : lo_selected[0].company_name,
        	in_log_id : lo_selected[0].log_id,
        	in_company_id : lo_selected[0].company_id,
        	in_publish_state : lo_selected[0].publish_state,
        	in_publish_date : lo_selected[0].publish_date,
        	in_article_url	: lo_selected[0].article_url,
        	in_article_title	:  lo_selected[0].article_title
        });
        var ls_url = getUrl();
        $('#dlg_inputPublish').show().dialog({width:390,height:290,fit:false,buttons: [{
            text:'确定',
            iconCls:'icon-ok',
            handler:function(){
            	$("#inputPublishForm").form("submit",{
            	        url: ls_url,
            			type : "post",
            			dataType : "text",
            			onSubmit : function(){
        ...
        }]}).dialog('open').dialog('setTitle','修改微信推广记录'); 
    }
}
/* ====================================*/
// 函数作用：（批量发送消息）
