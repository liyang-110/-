...
function getUrl() {
	var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=ad_trade_weixin_task&in_r=ad&in_URLEncoding=UTF-8"+querystr;
	return ls_url;	
}
...
function inputPublish(){
    ...
    if(lo_selected.length!=1){
        ...
        $("#inputPublishForm").form("load",{
        	...
        	in_publish_date : (new Date()).Format("yyyy-MM-dd")
        });
        var ls_url = getUrl();
        ...
        }]}).dialog('open').dialog('setTitle','录入微信推广记录'); 
    }
}
/* ====================================*/
// 函数作用：（批量发送消息）
