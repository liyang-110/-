...
function getUrl() {
	var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=admin_login_query&r=ad&in_type=employ&in_URLEncoding=UTF-8";
	return ls_url;	
	}
/* =======================================================*/
// 函数作用：打开新增页面
...
function add(){
	...
	$("#if_deal")[0].src = "./jLoginAdd2.jsp?pagesize=" + li_pagesize + "&pagenum=" + li_pagenum + "&currentnum=" + li_currentnum;
...
function edit() {
    ...
    if(lo_selected.length!=1){
        ...
        li_pagenum = lo_pg.pagination('options').pageNumber;
		li_pagesize = $('#resultId').datagrid('options').pageSize;// lo_pg.pagination("options").pageSize;
        $('#if_deal')[0].src='./jLoginEdit2.jsp?id='+ls_id+'&pagesize=' + li_pagesize+'&pagenum=' + li_pagenum+'&currentnum=' +li_currentnum +"&date="+new Date();
		$('#dlg_deal').dialog({width:480,height:300,fit:true,resizable:true});
