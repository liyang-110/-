...
function getUrl() {
		var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=ad_01&in_r=ad&in_ad_type=friendlink";
		return ls_url;	
	}
/* =======================================================*/
// 函数作用：打开新增页面
...
function add(){
	...
	$("#if_deal")[0].src =  "./jADAdd.jsp?pagesize=" + li_pagesize + "&pagenum=" + li_pagenum + "&currentnum=" + li_currentnum;
...
function edit() {
    ...
    if(lo_selected.length!=1){
        ...
        $('#if_deal')[0].src='./jADEdit.jsp?id='+ls_id+'&pagesize=' + li_pagesize+'&pagenum=' + li_pagenum+'&currentnum=' +li_currentnum +"&date="+new Date();
