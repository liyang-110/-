		...
		columns : [ [
		 {
			...
			formatter : function(value,row,index){
				return '<span style="color:'+row.ad_color+';">'+value+'</span>';
			}
		},{
			field : 'ad_pic',
			title : '图片',
...
function getUrl() {
		var ls_url ="";
		ls_url = cur_path + "/esbServer.do?method=execProc&methodId=ad_01&in_ad_type=text_ad";
		return ls_url;	
	}
/* =======================================================*/
// 函数作用：打开新增页面
...
function add(){
	...
	$("#if_deal")[0].src =  "jTextADAdd.jsp?pagesize=" + li_pagesize + "&pagenum=" + li_pagenum + "&currentnum=" + li_currentnum;
...
function edit() {
    ...
    if(lo_selected.length!=1){
        ...
        $('#if_deal')[0].src='jTextADEdit.jsp?id='+ls_id+'&pagesize=' + li_pagesize+'&pagenum=' + li_pagenum+'&currentnum=' +li_currentnum +"&date="+new Date();
