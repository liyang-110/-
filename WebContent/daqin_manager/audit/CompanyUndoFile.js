		...
		columns : [ [
		  {
			...
			formatter : function(value,row,index){
				...
				}else if( row.apply_type == 'school' ){
					return '<a href="'+cur_path+'/school/info.do?id='+row.apply_personid+'" target="_blank" title="'+value+'">'+value+'</a>';
				}
			},
			width: 200
		},{
			...
			title : '受邀企业',
			formatter : function(value,row,index){
				// row.apply_type == 'school'
				if( row.attend==1 ){
					return '<a href="'+cur_path+'/company/info.do?id='+row.deptid+'" target="_blank" title="'+value+'"><font color="green">'+value+'</font></a>';
				}else{
			...
			formatter:function(value,row,index){
				...
				if (value==null || value==''){
					...
					return btnStr+'剩余展位：'+row.last_company_num;
				}else if(value=='ok'){
...
function getUrl() {
	var ls_url ="";
		ls_url = cur_path + "/esbServer.do?in_r=ad&method=execProc&methodId=ad_file_undosearch&in_r=ad&in_apply_type=school&in_id="+id+"&in_user_id="+user_id;
	return ls_url;	
	}
