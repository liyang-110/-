...
function JumpSearchLayer(arrayid,fname,vname,tname,_maxSelectNumber){   
	...
	if (arrayid==1){
	    ...
	    dialog.title('请选择希望岗位类别（最多可选择3个）');
	}
	if (arrayid==2)
	{
	...
	if(arrayid==2){
		$("#bigselect").find("option[value='28']").attr("selected",true);
		BigChangeToSmall('陕西','28',2)
...
{	
	...
	if (selectokobj != null)
	{
        ...
        if (selectoknum >= maxSelectNumber)
		{
			...
			alert("对不起，您选中的选项已经超过"+maxSelectNumber+"项了，本系统中要求岗位唯一性，如果要填写多个，请继续增加简历！");		
			return false;
		}
/**        maxSelectNumber		 */		
	}
