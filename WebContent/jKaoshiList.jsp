...
<div class="area_select_link">
	<a <%=area_id==null?"class=\"active\"":"" %> href="?t=kaoshi<%=sParam %>">全国站</a>
					<%
                    ...
                    for( Iterator it=areaList.iterator();it.hasNext();)
                    {
                     ...
                     %>
                     <a class="active" href="?t=kaoshi&a=<%=row.get(0) %><%=sParam %>"><%=row.get(1) %></a>
                     <% }else{ %>
                     <a href="?t=kaoshi&a=<%=row.get(0) %><%=sParam %>"><%=row.get(1) %></a>
        ...
        <div class="left_b" style="margin:10px 0px">
          ...
          <%
                 newsModel= new NewsModel();
                  List newsList1= newsModel.getPageNewsList("news_type02", null,null,null, 1,15);
 ...
 <li><a href="<%=request.getContextPath() %>/newsDetail.do?id=<%=row.get("news_id") %>"  target="_blank" alt="<%=row.get("news_title") %> " title="<%=row.get("news_title") %> ">
    ...
    <div class="right">
        <div class="menu2">
            <ul>
                <li ><a href="<%=contextPath %>/list.do?t=gonggao<%=aParam %>">最新资讯</a></li>
                	<li  class="hotel" >
								<a href="<%=contextPath %>/list.do?t=kaoshi&s=shiyedanwei<%=aParam %>">最新招聘</a>
							</li>
							<!-- 
							<li <%="guoqizp".equals(sub_type)?"class=\"hotel\"":""%> >
