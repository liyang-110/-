...
function getData() {
	$('#resultId').datagrid( {
		...
		columns : [ [ {
			...
			formatter: function(value,row,index){
				if( value ){
					...
					return '<button onclick="grabOrder(\''+row.apply_id+'\','+index+');">抢单</button>';
				}
			}
		},{
			field : 'remark',
			...
			formatter : function(value,row,index ){
				if( value=='' || value == null ){
					...
					return '<a href="javascript:;" onclick="dealApply(\''+row.apply_id+'\',\''+row.company_name+'\','+index+');">'+value+'</a>';
				}
			}
		}
		] ], 
		...
		onDblClickRow : function(index ,row ){
			...
			var content = '<div class="detail-dlg" style="height:280px;width:605px;overflow:scroll;">'+
...
function companyDetail()
{
	...
	var introduce = row.introduce;
	var recruit = row.recruit;
	var content = '<div style="height:280px;width:605px;overflow:scroll;"><p>简介</p><p>'+introduce+'</p><p>招聘:</p><p>'+recruit+'</p>';
	// $.messager.alert(row.company_name,content );
	$("#dd").dialog({
	    title:row.company_name,   
	    width: 620,   
...
function dealApply( apply_id, company_name,index,remark ){
...
function grabOrder( apply_id,index ){
	
    var lo_pg         = $("#resultId").datagrid("getPager");
    var li_pagenum    = lo_pg.pagination('options').pageNumber;
	var li_pagesize = lo_pg.pagination("options").pageSize;
	
   var 	ls_url = cur_path + "/esbServer.do?method=execProc&methodId=job_fair_apply&r=ad&in_action=grab&in_apply_id="+apply_id+"&in_URLEncoding=UTF-8&date="+new Date();
