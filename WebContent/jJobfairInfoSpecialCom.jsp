...
<body>
	...
	<div class="container f_content">
		...
		<table width="800" border="0" cellpadding="5" cellspacing="0">
		  <tr>
          	<td>举办单位： <a href="<%=request.getContextPath() %>/company/info.do?id=<%=companyInfo.get("company_id") %>" target="_blank">
			<%=companyInfo.get("company_name") %>
			</a> </td>
			<td>举办时间：<%=jobfairInfo.get("job_fair_time") %> 至 <%=jobfairInfo.get("job_fair_overtime") %> </td>
		  </tr>
		  <tr>
          	<td>预定截止：<%=jobfairInfo.get("job_fair_time") %></td>
	...
	<div class="title1">	
			  ...
			  <div id="typ1" class="content">
					<% if("import".equals( jobfairInfo.get("add_person") )){ %>
						...
						%>
							...
							<div class="introduce">
						         	<%=companyInfo!=null?com.zhaopin.dao.factory.JobFairModel.textToHtml(
				...
				<div id="typ2" class="content" style="display:none;">
				 <%
         	List<HashMap> recruitList = (List<HashMap>)request.getAttribute("recruitList");
		...
		<table class="table table-bordered table-hover  recruitList">
			...
			<%
			  if( recruitList!=null && recruitList.size()>0 ) {
			  		for(HashMap map : recruitList ){
			 %>
	         <tr>
	         <td><%=map.get("job_type") %></td>
	         <td><a target="_blank" href="/company/jobDetail.do?id=<%=map.get("job_id") %>">
	         <%=map.get("job_name") %></a>
	         </td>
	         <td>
		            		<%
		            			String professional = (String)map.get("professional");
		            			if(professional==null || "".equals(professional ) ){
		            					out.println("专业不限");
		            			}else{
		            					String[] professionalArray = professional.split(",");
		            					for(int ii=0 ; ii<professionalArray.length&& ii<5 ; ii++){
		            						out.println( professionalArray[ii]+"<br/>");
		            					}
		            			}
		            		%>
	         </td>
	         <td><%=map.get("job_person_num") %></td>
	         <td><%=map.get("education") %></td>
	         <td><%=map.get("areas") %></td>
	         <td><%=map.get("money") %></td>
	         <td>
		            <a href="javascript:;" onclick="openOrderJob('<%=map.get("company_id") %>','<%=map.get("job_id") %>');"  style="font-size:12px !important;" >投递简历</a>
	         </td>
	         </tr>
			 <%
			 		}
