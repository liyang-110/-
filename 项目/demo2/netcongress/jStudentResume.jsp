	...
	<div class="caidan caidan_a">
    	<ul>
       	<li><a href="/netcongress/start.do?id=<%=job_fair_id %>" >招聘大厅</a></li>
            <li><a href="/netcongress/start.do?id=<%=job_fair_id %>&m=resume"   class="on">已投简历</a></li>
    ...
    <div class="cont_c">
       ...
       if(resumeList!=null && resumeList.size()>0){
       		...
       		for(HashMap row: resumeList ){
       			...
       			String tip="";
       			if( 0==status ){
       				tip="投递时间："+row.get("add_time")+"<br/>应聘岗位："+row.get("job_name");
       			}else{
       				tip="应聘单位："+row.get("company_name") +
       							"<br/>应聘岗位："+row.get("job_name")+
       							"<br/>通知时间："+row.get("deal_time")+
				            	"<br/>通知内容："+row.get("remark");
       			}
       	%>
            	<li class="interviewLi" title="<%=tip%>">
            	<p class="left">
            	<a target="_blank"  href="/company/info.do?id=<%=row.get("company_id")%>">
            	<%=row.get("company_name") %>
            	</a>
            	</p>
            	<p class="qq">
            	<% if(0!=status && row.get("bind_qq")!=null){ %>
...
function interviewLiDom( data ){
	var html = $("#interviewLi").text()
					...
					.replace(/{{deal_time}}/g,data.deal_time);
	return html;
}
function interviewStatus( status ){
		switch( status ){
			...
			case '4' :
				return '<span class="green">通知复试</span>';
			case '5' :
...
function ajaxMore( dom )
{
	...
	var url = cur_path+"/netcongress/ajaxMore.do?m=resume&p="+pageNum+"&s="+pageSize;
