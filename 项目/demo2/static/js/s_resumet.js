...
function makeCenter()
{
	...
	$('#choose-box-wrapper').css("left", Math.max(0, (($(window).width()-250 - $('#choose-box-wrapper').outerWidth()) / 2) + $(window).scrollLeft()) + "px");
...
function inputSchoolName()
{
	...
	if(school_name==''){
		...
		$("input[name='in_college']").val( school_name );
		$('#in_school_id').val('');
		hide();
	}
}
				...
				if(data.queryresult[0].ok=='ok'){
					if( data.queryresult[0].status ==1 ){
						$("#open_switch_btn").bind("click",function(){
							Resume.setOpen(0);
						}).text("").css({"color":"#02c08e"}).attr("title","点击隐藏简历, 企业将无法搜索到您的简历").attr("checked",false);
					}else{
						$("#open_switch_btn").bind("click",function(){
							Resume.setOpen(1);
						}).text("").css({"color":"#02c08e"}).attr("title","委托投递简历,等待HR来找你").attr("checked",true);
					}
				}else{
					$.messager.alert('提示信息','操作失败','error');
				}
...
var Work = {
			...
			$.ajax({
				...
				if (re.length > 0 && re[0].returncode == "ok" 
					...
					for(var i=0;i<initdata.length;i++){
						tr='<div class="work-experience">'+
						   '<span class="biaoti">公司名称：</span><span>'+initdata[i].company_name+'</span>'+
						   '<span style="float: right;">'+
						   '<a href="javascript:;" onclick="Work.edit(\''+initdata[i].work_id+'\');"><i class="fa fa-pencil-square-o work_img" style="display:none;color:#02c08e;"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
						   '<a href="javascript:;" onclick="Work.del(\''+initdata[i].work_id+'\');"><i class="fa fa-trash work_img" style="display:none;color:#02c08e;margin-right: 10px;"></i></a></span><br/>'+
						   '<span class="biaoti">岗位名称：</span><span>'+initdata[i].position+'</span></br>'+
						   '<span class="biaoti">时&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;间：</span><span style="text-align:left;">'+initdata[i].begin_date+'--'+initdata[i].end_date+'</span></br>'+
						   '<span class="biaoti">工作描述：</span><span><textarea class="auto-height gztextarea" style="border:none;color:#999;" readonly="readonly" name="in_introduce" cols="35" rows="2">'+initdata[i].introduce+'</textarea></span></div> ';
							$('.work_two').append(tr);	
				...
				//工作编辑(删除)按钮显示与隐藏
				$(".work-experience").mouseenter(function(){
					   $(this).find(".work_img").show();
					});
					 $(".work-experience").mouseleave(function(){
						 $(this).find(".work_img").hide();
						  });
		...
		del :function ( work_id )
		{
			var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=student_work&in_r=su&in_action=del&in_work_id="+work_id+"&date=" + new Date();
			$.ajax({
				url:ls_url, 
				type:"post", 
				dataType:"text", 
		...
		{
			var ls_url = cur_path + "/esbServer.do?method=execProc&methodId=student_work&in_r=su&in_action=view&in_work_id="+work_id+"&date=" + new Date();
			$.ajax({
				url:ls_url, 
				type:"post", 
				dataType:"text", 
...
var Review = {
		edit:function( action ){
			if( action==true ){
				$(".stu_review").hide();
				$("#stu_review_form_btn").hide();
				$("#stu_review_form").show();
				$("#stu_review_card_btn").show();
				
			}else{
				$(".stu_review").show();
				$("#stu_review_card_btn").hide();
				$("#stu_review_form").hide();
				$("#stu_review_form_btn").show();
			}
			$(".auto-height").trigger("change");
		},
		save: function(){
			$('#stu_review_form').form("submit",{
				success: function(data){
					data = decodeURIComponent(data);
					var re = $.parseJSON( data );
					//console.info( re );
					if( re.returncode=='ok' ){
						if( re.querycount==1 ){
							//console.info( re.queryresult[0] );
							$("#stu_review_card").form("load",re.queryresult[0] );
							Review.edit(false);
							alert("保存成功");
							window.location.reload();
						}else{
			...
			$.ajax({
				...
				if (re.length > 0 && re[0].returncode == "ok"
					...
					for(var i=0;i<initdata.length;i++){
						...
						tr='<div class="education-background">'+
							'<span class="biaoti">毕业院校 : </span><span style="text-align:left;">'+initdata[i].school_name+'</span><span style="float: right;">'+
							'<a href="javascript:;" onclick="Education.edit(\''+initdata[i].edu_id+'\');"><i class="fa fa-pencil-square-o edu_img" style="display:none;color:#02c08e;"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+
							'<a href="javascript:;" onclick="Education.del(\''+initdata[i].edu_id+'\');"><i class="fa fa-trash edu_img" style="display:none;color:#02c08e;margin-right:10px;"></i></a></span></br>'+
							'<span class="biaoti">所学专业 : </span><span>'+initdata[i].profession+'</span></br><span class="biaoti">时&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;间 : </span><span>'+initdata[i].begin_date+'-'+initdata[i].end_date+'</span></br><span class="biaoti">学&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;历 : </span><span>'+initdata[i].education+'</span></div>';
						$('.edu_two').append(tr);
					}
				}
				//教育编辑(删除)按钮显示与隐藏
				$(".education-background").mouseenter(function(){
						$(this).find(".edu_img").show();
					});
					$(".education-background").mouseleave(function(){
						$(this).find(".edu_img").hide();
						});     
			}});
		},
...
function checkMaxInput(obj, maxLen) {  
    ...
    if (obj.value.length > maxLen) { //如果输入的字数超过了限制  
        ...
        strResult = '<p  style="margin:-10px 0 0 46px;" id="' + newid + '" class=\'Max_msg\' ><br/>剩(' + (maxLen - obj.value.length) + ')字</p>'; //计算并显示剩余字数  
    }  
    else {  
        strResult = '<p  style="margin:-10px 0 0 46px;" id="' + newid + '" class=\'Max_msg\' ><br/>剩(' + (maxLen - obj.value.length) + ')字</p>'; //计算并显示剩余字数  
    }  
