...
function openOrderJob(company_id,job_id,company_name,job_name,contact_tel){
	$.post("/student/sendResume.do",{job_id,job_id}
	).done(function(re){
		console.log( re );
		if( 'nologin'== re ){
//			msg_alert("提示信息","请登录","warning");
			open_login_layer();
		}else if( 'succ'== re ){
			msg_alert("提示信息","投递成功","succeed");
		}else{
			msg_alert("提示信息",re,"error");
		}
	}).fail(function(){
		msg_alert("提示信息","网络连接失败","error");
	});
	return;
//	art.dialog.open("/service/personOrderV2.jsp?job_fair_id=&school_id=&company_id="+company_id+"&job_id="+job_id+"&company_name="+company_name+"&job_name="+job_name+"&contact_tel="+contact_tel, 
//						{title: '投递简历',
//						 height:350,
//						 width:320,	
//						 fixed:true,
...
function openOrderJobSetTop(company_id,job_id,top){
	openOrderJob(company_id,job_id);	
//	art.dialog.open("/service/personOrderV2.jsp?job_fair_id=&school_id=&company_id="+company_id+"&job_id="+job_id, 
//						{title: '投递简历',
//						 height:290,
//						 width:320,	
//						 top:top,	
...
function openOrder(job_fair_id,school_id,company_id){
	...
	art.dialog.open("/service/personOrderV2.jsp?job_fair_id="+job_fair_id+"&school_id="+school_id+"&company_id="+company_id, 
						{title: '投递简历',
						 height:290,
						 width:320,
						 fixed:true,
...
function open_login_layer(){
	art.dialog.open("/static/service/index_Login.jsp", 
						{title:false,
						 height:410,
						 width:450,	
						 fixed:true,
...
function quick_apply_form( job_fair_id ){
	art.dialog.open("/static/service/quickApplyFormSimple.jsp?id="+job_fair_id, 
						{title:false,
						 height:235,
						 width:400,	
						 fixed:true,
...
function upload_resume(){
	var ls_url='/student/stuUpload.do';
	...
	var pageContentBody =$(art.dialog({id: 'upload_resume_btn'}).content());
	pageContentBody.parents(".aui_dialog").find(".aui_close").addClass("stu_resumet_close");
